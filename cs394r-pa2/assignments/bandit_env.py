...
class BanditEnvironment(gym.Env):
    ...
    def _sample_reward(self, action: int) -> float:
        ...
        """
        # Standard deviation is 1 (variance = 1) as specified in Sutton
        # Generate a single sample from N(Q_star[a], 1)
    ...
    def _is_ideal_action(self, action: int) -> bool:
        ...
        """
        # The action is ideal if its true value is equal to the maximal Q* among all arms.
        # We use np.isclose to avoid floating-point comparison issues.
        max_q = np.max(self.Q_star)
    ...
    def _walk_all_arms(self):
        ...
        noise ~ N(0, 0.01)
        """
        # Add Gaussian noise with mean 0 and std dev 0.1 (variance 0.01) to each arm
        noise = np.random.normal(loc=0.0, scale=0.01, size=self.Q_star.shape)
        self.Q_star += noise
