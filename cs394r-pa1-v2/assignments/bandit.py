...
class SampleAverageMethod(ActionValue):
    ...
    def update(self, a: int, r: float):
        ...
        Parameters:
            ...
            r (float): the observed reward        
        """
        # Increment count for the selected action and update its estimated
        # value using the running sample average.
        self.n[a] += 1
        step_size = 1.0 / self.n[a]
        self.Q_hat[a] += step_size * (r - self.Q_hat[a])
...
class ConstantStepSizeMethod(ActionValue):
    ...
    def update(self, a: int, r: float) -> None:
        ...
        Parameters:
            ...
            r (float): the observed reward
        """
        # Exponential recency-weighted average with constant step size alpha.
...
class BanditSolver(Solver):
    ...
    def train_episode(self):
        ...
        while not done and not truncated:
            # 1) Select an action using epsilon-greedy policy
            a = policy.action(None)
            # 2) Step the bandit environment
            _, r, done, truncated, info = env.step(a)
            # 3) Update the action-value estimate
            method.update(a, r)
