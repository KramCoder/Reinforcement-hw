...
class BanditPolicy(Policy):
    ...
    def action(self, _: int = None) -> int:
        ...
        if np.random.rand() < self.epsilon:
            return int(np.random.randint(0, len(self.Q)))

        # Otherwise choose one of the greedy actions. When multiple actions
        # tie for the maximum estimated value, break ties uniformly at random.
        max_q = self.Q.max()
