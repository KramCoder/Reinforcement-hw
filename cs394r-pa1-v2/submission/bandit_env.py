...
class BanditEnvironment(gym.Env):
    ...
    def _sample_reward(self, action: int) -> float:
        ...
        # Sample from normal distribution with mean Q*(action) and standard deviation 1.0
    ...
    def _is_ideal_action(self, action: int) -> bool:
        ...
        max_q_value = np.max(self.Q_star)
        # Check if the selected action has the maximum Q* value
        return self.Q_star[action] == max_q_value
    ...
    def _walk_all_arms(self):
        ...
        # Add random walk noise to all Q* values
        # noise ~ N(0, 0.01^2), so mean=0, std=0.01
        noise = np.random.normal(0, 0.01, size=len(self.Q_star))
        self.Q_star += noise